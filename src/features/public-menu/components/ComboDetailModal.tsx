    ...
    >
      ...
      >
        ...
        <div className="flex items-stretch gap-0">
          {promo.imagen_url && (
            <div className="w-24 shrink-0 overflow-hidden bg-[var(--color-custom-100)]">
              <img
          ...
          <div className="flex-1 min-w-0 px-4 py-3.5">
            <div className="flex items-center gap-2 mb-1">
              <div className="flex h-5 w-5 items-center justify-center rounded-lg bg-[var(--color-custom-500)] text-white">
                <ShoppingBag size={10} />
              </div>
              <span className="rounded-full bg-[var(--color-custom-500)]/10 px-1.5 py-[1px] text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--color-custom-600)]">
                Combo
              </span>
            </div>
            <h3 className="text-sm font-bold text-[var(--color-custom-900)] leading-tight">
              {promo.nombre}
            </h3>
            {promo.descripcion && (
              <p className="mt-0.5 text-[11px] text-[var(--color-custom-text-muted)] leading-relaxed line-clamp-2">
                {promo.descripcion}
              </p>
            )}
          </div>
          <button
            ...
            onClick={onCancel}
            aria-label="Cerrar"
            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg m-3 text-[var(--color-custom-text-muted)] transition-colors hover:bg-[var(--color-custom-100)]"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="px-4 pb-2 space-y-3">
          <div className="divide-y divide-[var(--color-custom-border)] rounded-xl border border-[var(--color-custom-border)] overflow-hidden">
            {items.map((item, idx) => (
              ...
              >
                ...
                <span className="flex-1 text-xs font-medium text-[var(--color-custom-900)] truncate">
          ...
          <div className="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-custom-border)] bg-[var(--color-custom-100)]/50 px-3.5 py-2.5">
            <div className="space-y-0.5">
              <p className="text-[10px] text-[var(--color-custom-text-muted)] line-through">
                {formatMoney(realTotal, simbolo)}
              </p>
              <p className="text-lg font-black text-[var(--color-custom-500)] leading-none">
                {formatMoney(Number(promo.valor_descuento), simbolo)}
              </p>
            </div>
            {ahorro > 0 && (
              <div className="shrink-0 rounded-full bg-green-500/10 px-2.5 py-1 text-[10px] font-bold text-green-600">
        ...
        {/* Footer */}
        <div className="border-t border-[var(--color-custom-border)] px-4 py-3 flex items-center justify-between gap-3">
          ...
          <motion.button
            ...
            onClick={handleAdd}
            whileTap={{ scale: 0.98 }}
            className="flex items-center gap-1.5 rounded-xl bg-[var(--color-custom-900)] px-4 py-2.5 text-xs font-bold text-white hover:bg-[var(--color-custom-800)] transition-colors shadow-sm"
          >
            <ShoppingBag size={14} />
