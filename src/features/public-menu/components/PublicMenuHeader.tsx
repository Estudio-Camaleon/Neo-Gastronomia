...
export function PublicMenuHeader({
  ...
  // Mapa de redes sociales: id -> URL, label, icono, color hover
  const SOCIAL_MAP: Record<string, { label: string; ariaLabel: string; getUrl: (n: NegocioPublico) => string | null; hoverBg: string; icon: ReactNode }> = useMemo(() => ({
    instagram: {
      label: "Instagram",
      ariaLabel: "Instagram",
      getUrl: (n) => n.instagram_url,
      hoverBg: "#E4405F",
  ...
  const primaryNetworks = (negocio.redes_principales ?? []).filter(
    (id) => SOCIAL_MAP[id] && SOCIAL_MAP[id].getUrl(negocio),
  );

  const hasSocials =
    negocio.whatsapp || primaryNetworks.length > 0;
            ...
            >
              ...
              {hasSocials && (
                ...
                >
                  ...
                  {/* Solo las redes marcadas como principales en la configuración */}
                  {primaryNetworks.map((id) => {
                    const red = SOCIAL_MAP[id];
                    if (!red) return null;
                    return (
                      <a
                        key={id}
                        href={red.getUrl(negocio)!}
                        target="_blank"
                        rel="noreferrer"
                        aria-label={red.ariaLabel}
                        style={{ "--hover-bg": red.hoverBg } as React.CSSProperties}
                        className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white/80 transition-all hover:bg-[var(--hover-bg)] hover:text-white hover:scale-110 active:scale-95"
                      >
                        {red.icon}
                      </a>
                    );
